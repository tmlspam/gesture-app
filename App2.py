...
class GestureController:
    def __init__(self):
        ...
        # Control settings with optimized defaults
        self.sensitivity = 0.7
        self.smoothing = 0.3  # Less smoothing for faster response
        ...
        self.empty_frame = np.zeros((self.cam_height, self.cam_width, 3), dtype=np.uint8)
        
        # Initialize GUI
        self.setup_gui()

    def setup_gui(self):
        """Optimized GUI setup with minimal elements"""
        dpg.create_context()
        dpg.create_viewport(title='Gesture Control Pro', width=800, height=600)
        
        with dpg.texture_registry(show=False):  # Hide registry
        ...
        with dpg.window(label="Control Panel", tag="main_window"):
            with dpg.group(horizontal=True):
                # Camera view
                with dpg.child_window(width=400, tag="camera_view"):
                ...
                with dpg.child_window(width=400):
                    ...
                    with dpg.collapsing_header(label="Settings", default_open=True):
                        dpg.add_slider_float(
                            ...
                            default_value=self.sensitivity, 
                            callback=lambda s, d: setattr(self, 'sensitivity', d)
                        )
                        dpg.add_slider_float(
                            label="Smoothing", 
                            min_value=0.0, max_value=1.0, 
                            default_value=self.smoothing, 
                            callback=lambda s, d: setattr(self, 'smoothing', d)
                        )
                    ...
                    with dpg.group(horizontal=True):
                        dpg.add_button(
                            label="Mouse", 
                            callback=lambda: self.set_control_mode("mouse"),
                            width=100
                        )
                        dpg.add_button(
                            label="Media", 
                            callback=lambda: self.set_control_mode("media"),
                            width=100
                        )
                        dpg.add_button(
                            label="Keyboard", 
                            callback=lambda: self.set_control_mode("keyboard"),
                            width=100
                        )
    ...
    def process_frame(self, frame):
        """Highly optimized frame processing pipeline"""
        # Flip and convert color space in one operation
        ...
        if results.multi_hand_landmarks:
            ...
            # Get landmarks with minimal overhead
            landmarks = {
            ...
            # Detect gesture (optimized calculations)
    ...
    def _detect_gesture(self, landmarks):
        """Optimized gesture detection with vectorized operations"""
        ...
        thumb_index_dist = np.linalg.norm(
            np.array(landmarks['thumb']) - np.array(landmarks['index']))
    ...
    def _execute_control(self, landmarks):
        ...
        # Apply smoothing with numpy for performance
    ...
    def run(self):
        ...
        cap.set(cv2.CAP_PROP_FPS, 30)  # Limit to 30 FPS for consistency
        ...
        while self.running and cap.isOpened():
            ...
            # Process frame and get current time
